...
func (d *Data) sendBlocksSpecific(ctx *datastore.VersionedCtx, w http.ResponseWriter, supervoxels bool, blockstring string, scale uint8) error {
...
func (d *Data) ServeHTTP(uuid dvid.UUID, ctx *datastore.VersionedCtx, w http.ResponseWriter, r *http.Request) {
	...
	case "specificblocks":
		...
		supervoxels := r.URL.Query().Get("supervoxels") == "true"
		...
		if action == "get" {
			if err := d.sendBlocksSpecific(ctx, w, supervoxels, blocklist, scale); err != nil {
				server.BadRequest(w, r, err)
				return
			}
			timedLog := dvid.NewTimeLog()
