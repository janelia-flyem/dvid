...
POST <api URL>/node/<UUID>/<data name>/raw/0_1_2/<size>/<offset>[?queryopts]
    ...
    Ingests block-aligned voxel data using the block sizes defined for this data instance.  
...
GET <api URL>/node/<UUID>/<data name>/supervoxel[?queryopts]

	Returns JSON for the supervoxel at a list of coordinates.  Expects JSON in GET body:
...
GET <api URL>/node/<UUID>/<data name>/supervoxels[?queryopts]
	...
	Returns JSON for the supervoxels at a list of coordinates.  Expects JSON in GET body:
...
POST <api URL>/node/<UUID>/<data name>/split-supervoxel/<supervoxel>
	...
	NOTE 1: The POSTed split sparse volume must be a subset of the given supervoxel.  You cannot
	give an arbitrary sparse volume that may span multiple supervoxels.
...
func (d *Data) getLabelBlock(ctx *datastore.VersionedCtx, scale uint8, bcoord dvid.IZYXString) (*labels.PositionedBlock, error) {
	store, err := datastore.GetKeyValueDB(d)
	if err != nil {
		return nil, fmt.Errorf("labelmap getLabelBlock() had error initializing store: %v\n", err)
	}
	tk := NewBlockTKeyByCoord(scale, bcoord)
	val, err := store.Get(ctx, tk)
	if err != nil {
		return nil, fmt.Errorf("Error on GET of labelmap %q label block @ %s\n", d.DataName(), bcoord)
	}
	if val == nil {
		return nil, nil
	}
...
func (d *Data) putLabelBlock(ctx *datastore.VersionedCtx, scale uint8, pb *labels.PositionedBlock) error {
	store, err := datastore.GetKeyValueDB(d)
	if err != nil {
		return fmt.Errorf("labelmap putLabelBlock() had error initializing store: %v\n", err)
	}
...
func (d *Data) sendBlock(w http.ResponseWriter, x, y, z int32, v []byte, compression string) error {
	...
	case dvid.Uncompressed, dvid.Gzip:
		...
		out = v[start:]
	default:
		return fmt.Errorf("labelmap data was stored in unknown compressed format: %s\n", formatIn)
	}
...
func (d *Data) handleDataRequest(ctx *datastore.VersionedCtx, w http.ResponseWriter, r *http.Request, parts []string) {
	...
	case 3:
		...
		if strings.ToLower(r.Method) == "get" {
			...
			if err = d.PutLabels(ctx.VersionID(), subvol, data, roiname); err != nil {
				server.BadRequest(w, r, err)
				return
			}
		}
...
func (d *Data) handleCleave(ctx *datastore.VersionedCtx, w http.ResponseWriter, r *http.Request, parts []string) {
	...
	w.Header().Set("Content-Type", "application/json")
	fmt.Fprintf(w, `{"CleaveLabel": %d}`, cleaveLabel)
