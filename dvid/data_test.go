...
func (s *DataSuite) TestPoint3d(c *C) {
	...
	g := d.Chunk(blockSize)
	c.Assert(g, Equals, Chunk{5, 7, 16})

	d = Point3d{111, 213, 680}
	g = d.Chunk(blockSize)
	c.Assert(g, Equals, Chunk{5, 7, 17})

	d = Point3d{111, 213, 678}
	blockSize = Point3d{20, 30, 1}
	g = d.Chunk(blockSize)
	c.Assert(g, Equals, Chunk{5, 7, 678})
...
func (s *DataSuite) TestChunk(c *C) {
	...
	result = a.Max(b)
	c.Assert(result, Equals, Chunk{2980, 8191, 32001})
	result = b.Max(a)
	c.Assert(result, Equals, Chunk{2980, 8191, 32001})

	result = a.Min(b)
	c.Assert(result, Equals, Chunk{123, 617, 99})
	result = b.Min(a)
	c.Assert(result, Equals, Chunk{123, 617, 99})
