...
func (suite *DataSuite) TestNewDataDifferent(c *C) {
	...
	err = suite.service.NewData(root, "helloworld", "h1", config)
	c.Assert(err, IsNil)

	dataservice1, err := suite.service.DataService(root, "h1")
	c.Assert(err, IsNil)

	err = suite.service.NewData(root, "helloworld", "h2", config)
	c.Assert(err, IsNil)

	dataservice2, err := suite.service.DataService(root, "h2")
	c.Assert(err, IsNil)
